# --- SYSTEM ALIASES & COM INITIALIZATION ---
import sys
import numpy
try:
...
from src.cnn_gru_inference import CNNGRUDetector
...
# --- CNN-GRU Detector Instance ---
cnn_gru_detector = CNNGRUDetector()

# Example API endpoint for CNN-GRU detection
@app.route('/api/cnn_gru_detect', methods=['POST'])
def cnn_gru_detect():
    """
    Accepts JSON: {"sequence": [[...], [...], ...]} (sequence_length x num_features)
    ...
    try:
        ...
        is_mal, conf = cnn_gru_detector.predict(arr)
        return jsonify({"is_malicious": is_mal, "confidence": conf})
...
def api_system_logs():
    return jsonify(get_system_logs())
...
def download_pcap(filename):
    """Download PCAP file from logs/pcaps/ directory."""
    try:
        import os
        import re
        ...
        pcap_dir = os.path.join(os.path.dirname(__file__), "logs", "pcaps")
        pcap_path = os.path.join(pcap_dir, filename)
        
        # Verify file exists and is within pcaps directory
        if os.path.exists(pcap_path) and os.path.abspath(pcap_path).startswith(os.path.abspath(pcap_dir)):
            return send_file(pcap_path, as_attachment=True, download_name=filename)
        else:
            return {"error": "PCAP file not found"}, 404
    except Exception as e:
        return {"error": f"Failed to download PCAP: {str(e)}"}, 500
